...
\begin{figure}[!t]
	...
	\caption{\textcolor{red}{The plot shows the structure of a classifier, which is a recurrent Elman neural 
...
\subsubsection{\textcolor{red}{Optimization Algorithm}}\label{sec:optimization_algorithm_swarm}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=3.0 in]{classifier_fitness_simulation.pdf}
	\caption{This plot shows the average fitness of the classifiers over generations in $30$ coevolution runs. The fitness is calculated based on $14641$ testing models. See text for details.}
...
\begin{figure}[!t]
    ...
    \caption{\textcolor{red}{This plot shows the decision accuracy of the~\textit{classifier system (archive)} in the $1000^\mathrm{th}$ generation of $30$ coevolution runs for different numbers of agents in the group. In each case, $14641$ different models were tested. A single replica was present in each trial.}}
...
\begin{figure}[!t]%htbp
	...
	\caption{\textcolor{red}{This plot shows the decision accuracy of the~\textit{classifier system (archive)} in the $1000^\mathrm{th}$ generation of $30$ coevolution runs with various number of classifiers chosen to form a system.}}
...
\textcolor{red}{As we mentioned before, we chose $10$ classifiers to form a classifier system to have a reasonable decision accuracy. Here we investigate how the classifier systems perform with various number of classifiers chosen to form a system. The decision accuracy of the~\textit{classifier system (archive)} is shown in Figure~\ref{fig:last_generation_judgment_accuracy_cf_num_selection}\footnote{The result of~\textit{classifier system (subjective)} is similar.}. It seems that as long as the number of classifiers chosen to form a system is within a certain range, the system could perform well. For example, when the number of selected classifiers is between $10$ and $25$, the classifier system can obtain a high decision accuracy. However, when the number is high (e.g., $75$), the decision accuracy declines dramatically.}
%
\begin{figure}[!t]%htbp
	\centering
...
\subsection{\textcolor{red}{Noise Study}}\label{sec:noise_study_swarm_simulation}
