...
%In order to make this setup more feasible to implement, it is assumed that the system cannot directly measure the speed of the individual, but rather its position.
...
Passive 2 (SP-EA) & $0$ & $0$ & $9.8$ & $48.5$ \\  % [1ex] adds vertical space 
CEA-IM & $4.0\times10^{-5}$ & $4.9\times10^{-5}$ & $1.1\times10^{-3}$ & $2.5\times10^{-4}$ \\  % [1ex] adds vertical space 
%\cline{4-5}
...
\end{figure}
%For all the other cases (e.g., $H/(1-p_1)$), the agent stays in the same state.
...
In this section, we present the results of inferring the 2-state agent behavior shown in Figure~\subref*{fig:stochastic_interaction_2states}, using the setups in Section~\ref{sec:simulation_setup_stochastic_interaction}. 

%compare and analyze the results obtained using the methods discussed in the previous section (\ref{sec:simulation_setup_stochastic_interaction}) for inferring the 2-state agent behavior shown in Figure~\subref*{fig:stochastic_interaction_2states}.
...
\begin{figure}[!t]%htbp
	\centering
		...
		\subfloat[SP-EA \label{fig:parameters_box_coevolution_metric}]{%
			\includegraphics[width=2.8 in]{parameters_box_coevolution_metric_2states.pdf}
		}\\
		\subfloat[CEA-IM \label{fig:parameters_box_evolution_metric}]{%
...
\begin{figure}[!t]
\centering
\includegraphics[width=4.5in]{classifier_interaction_3states.pdf}
...
%The results suggest that a machine could also exhibit such intelligent behavior as observed in human behavior, and this could pave the way to further development of machine intelligence. 
