...
\begin{figure}[!t]%htbp
	\centering
		...
		\caption{The stochastic behaviors with (a) 2 states and (b) 3 states under invesitgation in this thesis.\label{fig:stochastic_interaction_case_study}}
\end{figure}
...
For a proof-of-concept study, we assume that the agent moves in one-dimensional space and moves at a consistent speed in each state. Suppose the agent stays static and its initial state is known. The system identification task is to identify the parameters of the agent's other states (i.e., $v_1$ and $v_2$ shown in Figure~\ref{fig:stochastic_interaction_case_study}) and the two probabilities, $p_1$ and $p_2$. The speed of the agent in each state is chosen arbitrarily. $p_1$ and $p_2$ are chosen to have a value of $0.1$ and $1.0$. In this case, states $2$ and $3$ have low observability. In order to make the agent stay in the higher state number, the classifiers need to capture the point when the agent starts to jump from a state with a lower number to a state with a higher number and switches the light intensity immediately. By doing this, the agent would keep staying in the state with a higher number in a longer time, makes it easier for the machine to learn the behavior. If the classifiers fail to do that, the agent would immediately move to the previous state with lower number. 
...
The simulation setup used for learning the stochastic behaviors is the same with that in Section~\ref{sec:simulation_setup_deterministic}. For \textit{Turing Learning}, we still used three setups: ``Interactive'', ``Passive 1'' and ``Passive 2'', as discussed in Section~\ref{sec:simulation_setup_deterministic}. We also compared \textit{Turing Learning} with the two metric-based methods (SPEA and CoEAIM) described in Section~\ref{sec:single_population_EA} and Section~\ref{sec:coevolution_of_tests_and_models}, respectively. Note that for each setup, we added a certain amount of noise into the measure of speed. This is realized by multiplying the agent's real speed with a random value in $[0.95, 1.05]$ in each time step. 
...
\begin{table}[!t] 
\caption{This table shows a comparison of all approaches for inferring the 3-state agent behavior shown in Figure~\ref{fig:stochastic_interaction_case_study}\textcolor{blue}{(b)}. The values show means of AEs (defined in Equation~\ref{eq:AE} of Chapter~\ref{ch:swarm_simulation}) of each evolved model parameter with respect to that of the agent.} 
\renewcommand{\arraystretch}{1.1}
\centering % used for centering table 
\begin{tabular}{l l l l l l} % left justifying (4 columns) 
\hline\hline  %inserts double horizontal lines
...
This chapter presented the results of inferring deterministic and stochastic agent behaviors using using different system identification methods (\textit{Turing Learning}, SPEA, CoEAIM). In both behaviors, the results show that learning through interaction is better or faster than only through passive observation. When learning deterministic behaviors, both the ``Interactive'' setups of \textit{Turing Learning} and CoEAIM perform well in terms of the models obtained. 

However, the ``Interactive'' setup \textit{Turing Learning} performs significantly better than the two metric-based methods (SPEA and CoEAIM) in inferring stochastic behaviors. This shows the advantage of \textit{Turing Learning}. In the ``Interactive'' setup of \textit{Turing Learning}, the classifiers were shown to learn good strategies to interact with the agents. The results suggest that a machine could also exhibit the true interactive ability as observed in human behavior, and this could pave the way to further development of machine intelligence. 
